...
const App: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const profile = await analyzeProductContext(files);
  ...
  const handleGenerateLP = async () => {
    ...
    try {
      const lp = await generateSwipeLP(productProfile);
      setSwipeLP(lp);
    ...
    for (let i = 0; i < currentScreens.length; i++) {
        ...
        try {
            ...
            if (!currentScreens[i].designSpec) {
                const spec = await generateSingleDesignSpec(currentScreens[i], currentScreens, files, swipeLP.concept);
            ...
            if (!currentScreens[i].imageData && currentScreens[i].designSpec) {
                const base64Image = await generateSwipeScreenImage(currentScreens[i], files);
  ...
  const handleRegenerateScreen = async (index: number, instruction: string) => {
    ...
    try {
      const newScreen = await regenerateSwipeScreen(productProfile, originalScreen, instruction);
  ...
  const handleRegenerateVisual = async (index: number, instruction: string) => {
    ...
    try {
      ...
      const newSpec = await regenerateDesignSpec(screen, files, instruction);
      ...
      const base64Image = await generateSwipeScreenImage(updatedScreen, files);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans">
      <Header onApiKeyClick={handleSelectKey} />
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-10">
        ...
        {!hasApiKey && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center shadow-sm max-w-2xl mx-auto animate-in fade-in slide-in-from-top-4">
             ...
             <h3 className="text-lg font-bold text-gray-900 mb-2">APIキーの接続が必要です</h3>
             <p className="text-gray-600 mb-6">Gemini 3 Pro を使用するにはAPIキーを選択してください。</p>
            ...
            <div key={step.id} className="flex flex-col items-center bg-gray-50 px-2">
              <div 
                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                  isActive ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-gray-300 text-gray-400'
